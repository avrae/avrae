...
# signatures
SIG_SECRET = "this is a secret".encode()  # todo pull this out to config
SIG_STRUCT = struct.Struct('!QQQB')  # u64, u64, u64, u8 - https://docs.python.org/3/library/struct.html
SIG_HASH_ALG = hashlib.sha1  # SHA1 is technically compromised but the hash collision attack vector is not feasible here


def create_signature(ctx: disnake.ext.commands.Context, execution_scope: ExecutionScope, user_data: int):
    ...
    # encode
    packed = SIG_STRUCT.pack(ctx.message.id, ctx.channel.id, ctx.author.id, tail_byte)
...
def verify_signature(ctx: disnake.ext.commands.Context, data: str):
    ...
    try:
        ...
        message_id, channel_id, author_id, tail_byte = SIG_STRUCT.unpack(decoded_data)
    ...
    execution_scope = tail_byte & 0x07
