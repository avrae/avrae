...
#
#     def __init__(self, mdb, *args, **kwargs):
...
#
#     @classmethod
#     def new(cls, initial_names):
#         mdb = MongoClient(config.MONGO_URL)[config.MONGODB_DB_NAME]
#         return cls(mdb, builtins=DEFAULT_BUILTINS, initial_names=initial_names)
#
...
def run(string, bind_locals):
    """Main multiprocessing entrypoint to running an interpolation."""
    return f"!!echo ok it's running, string is `{string}` and locals are ```\n{bind_locals}\n```"
    # ops = r"([-+*/().<>=])"
    #
    # def evalrepl(match):
