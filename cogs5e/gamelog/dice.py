...
class DiceHandler(GameLogCallbackHandler):
    ...
    async def dice_roll(self, gctx, roll_request: ddb.dice.RollRequest):
        ...
        if not roll_request.rolls:  # do nothing if there are no rolls actually made
            ...
            len(roll_request.rolls) > 1
        ):  # if there are multiple rolls in the same event, just use the default handler
        ...
        await roll_callbacks.get(first_roll.roll_type, self.dice_roll_roll)(
            gctx, roll_request
        )
    ...
    async def dice_roll_roll(gctx, roll_request, comment=None, comment_getter=None):
        ...
        for rr in roll_request.rolls:
            results.append(
                str(
                    rr.to_d20(
                        stringifier=VerboseMDStringifier(), comment=comment_getter(rr)
                    )
                )
            )
    ...
    async def _dice_roll_embed_common(
        self, gctx, roll_request, title_fmt: str, **fmt_kwargs
    ):
        """
    ...
    async def dice_roll_heal(self, gctx, roll_request):
        ...
        await self._dice_roll_embed_common(
            gctx, roll_request, "{name} heals with {heal}!", heal=roll_request.action
        )
    ...
    async def dice_roll_spell(self, gctx, roll_request):
        ...
        await self.dice_roll_roll(
            gctx, roll_request, comment=f"{roll_request.action}: Spell"
        )
    ...
    async def dice_roll_to_hit(self, gctx, roll_request):
        ...
        if (caster := await gctx.get_statblock()) is None:
            await self.dice_roll_roll(
                gctx, roll_request, comment=f"{roll_request.action}: To Hit"
            )
            return
        ...
        if action is not None:
            ...
            embed = gamelogutils.embed_for_basic_attack(
                gctx, roll_request.action, caster, attack_roll
            )

        message = await gctx.send(embed=embed)
        if pend_damage:
            await gamelogutils.PendingAttack.create(
                gctx, roll_request, gctx.event, message.id
            )
    ...
    async def dice_roll_damage(self, gctx, roll_request):
        ...
        if (caster := await gctx.get_statblock()) is None:
            await self.dice_roll_roll(
                gctx, roll_request, comment=f"{roll_request.action}: Damage"
            )
            return
        ...
        action = await gamelogutils.action_from_roll_request(gctx, caster, roll_request)
        if action is not None:
            embed = gamelogutils.embed_for_action(
                gctx, action, caster, attack_roll, damage_roll
            )
        else:
            embed = gamelogutils.embed_for_basic_attack(
                gctx, roll_request.action, caster, attack_roll, damage_roll
            )

        # either update the old message or post a new one
        if pending is not None:
            partial = discord.PartialMessage(
                channel=await gctx.destination_channel(), id=pending.message_id
            )
            try:
