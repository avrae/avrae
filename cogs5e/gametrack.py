...
class GameTrack(commands.Cog):
    ...
    async def _rest(self, ctx, rest_type, *args):
        ...
        if "-h" in args:
            ...
            embed.add_field(
                name="Hit Points", value=f"{character.hp_str()}{hp_delta_str}"
            )
    ...
    @game.group(
        name="coinpurse", aliases=["coins", "coin"], invoke_without_command=True
    )
    ...
    async def game_deathsave(self, ctx, *args):
        ...
        embed.add_field(
            name="Death Saves", value=str(character.death_saves), inline=False
        )
    ...
    async def spellbook(self, ctx, *args):
        ...
        ep.add_field(
            name="Spell Attack Bonus", value=str(character.spellbook.sab), inline=True
        )
        ep.add_field(
            name="Spell Slots",
        ...
        prepared_count = sum(
            1 for spell in character.spellbook.spells if spell.prepared
        )
        ...
        if known_count == prepared_count:
            ...
            ep.add_description(
                f"{character.name} has {prepared_count} spells prepared and knows {known_count} spells."
            )
        ...
        for sb_spell in character.spellbook.spells:
            ...
            results, strict = search(
                choices, sb_spell.name, lambda sp: sp.name, strict=True
            )
            if not strict:
                known_level = "unknown"
                if len(results) > 1:
        ...
        if flag_show_prepared_help:
            footer_out.append(
                f'Unprepared spells were not shown. Use "{ctx.prefix}spellbook all" to view them!'
            )
    ...
    async def customcounter(self, ctx, name=None, *, modifier=None):
        ...
        *Ex:*
        ...
        if modifier is None:  # display value
            ...
            if counter.desc:
                counter_display_embed.add_field(
                    name="Description", value=counter.desc, inline=False
                )
    ...
    async def customcounter_create(self, ctx, name, *args):
        ...
        conflict = next(
            (c for c in character.consumables if c.name.lower() == name.lower()), None
        )
        if conflict:
            if await confirm(
                ctx,
                "Warning: This will overwrite an existing consumable. Continue? (Reply with yes/no)",
            ):
    ...
    async def customcounter_summary(self, ctx, page: int = 1):
        ...
        if character.consumables:
            ...
            if total > 25:
                embed.set_footer(
                    text=f"Page [{page}/{maxpage}] | {ctx.prefix}cc list <page>"
                )
        else:
            embed.add_field(
    ...
    async def cast(self, ctx, spell_name, *, args=""):
        ...
        if not args.last("i", type_=bool):
            try:
                spell = await select_spell_full(
                    ctx, spell_name, list_filter=lambda s: s.name in char.spellbook
                )
        ...
        if (gamelog := self.bot.get_cog("GameLog")) and result.automation_result:
            await gamelog.send_automation(
                ctx, char, spell.name, result.automation_result
            )
