...
class Homebrew(commands.Cog):
    ...
    async def bestiary(self, ctx, *, name=None):
        ...
        if len(monnames) < 1020:
            ...
            embed.add_field(
                name="Creatures", value=f"{len(bestiary.monsters)} creatures."
            )
    ...
    async def bestiary_import(self, ctx, url):
        ...
        bestiary = await Bestiary.from_critterdb(
            ctx, bestiary_id, published=is_published
        )
    ...
    async def bestiary_update(self, ctx):
        ...
        bestiary = await Bestiary.from_critterdb(
            ctx, old_bestiary.upstream, old_bestiary.published
        )
    ...
    async def bestiary_server(self, ctx):
        ...
        if is_server_active:
            ...
            await ctx.send(
                f"Ok, {bestiary.name} is no longer active on {ctx.guild.name}."
            )
    ...
    async def bestiary_server_list(self, ctx):
        ...
        await ctx.send(
            embed=discord.Embed(
                title="Active Server Bestiaries", description="\n".join(desc)
            )
        )
    ...
    async def bestiary_server_remove(self, ctx, bestiary_name):
        ...
        bestiary = await search_and_select(
            ctx, bestiaries, bestiary_name, lambda b: b.name
        )
    ...
    async def pack_list(self, ctx):
        ...
        await ctx.send(
            f"Your available packs: {', '.join([p['name'] async for p in available_pack_names])}"
        )
    ...
    async def pack_editor(self, ctx, user: discord.Member):
        ...
        if not await can_edit_editor(pack, ctx, user):
            return await ctx.send(
                "You do not have permission to add editors to this pack."
            )
    ...
    async def pack_unsub(self, ctx, name):
        ...
        try:
            ...
            return await ctx.send(
                "You aren't subscribed to this pack! Maybe you own it, or are an editor?"
            )
    ...
    async def pack_server_list(self, ctx):
        ...
        await ctx.send(
            embed=discord.Embed(title="Active Server Packs", description=desc)
        )
    ...
    async def pack_server_remove(self, ctx, pack_name):
        ...
        pack_meta = await search_and_select(
            ctx, pack_metas, pack_name, lambda b: b["name"]
        )
    ...
    async def tome_list(self, ctx):
        ...
        await ctx.send(
            f"Your available tomes: {', '.join([p['name'] async for p in available_tome_names])}"
        )
    ...
    async def tome_editor(self, ctx, user: discord.Member):
        ...
        if not await can_edit_editor(tome, ctx, user):
            return await ctx.send(
                "You do not have permission to add editors to this tome."
            )
    ...
    async def tome_unsub(self, ctx, name):
        ...
        try:
            ...
            return await ctx.send(
                "You aren't subscribed to this tome! Maybe you own it, or are an editor?"
            )
    ...
    async def tome_server_list(self, ctx):
        ...
        await ctx.send(
            embed=discord.Embed(title="Active Server Tomes", description=desc)
        )
    ...
    async def tome_server_remove(self, ctx, tome_name):
        ...
        tome_meta = await search_and_select(
            ctx, tome_metas, tome_name, lambda b: b["name"]
        )
