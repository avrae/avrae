...
class Effect:
    ...
    def __str__(self):
        return self.get_str(
            duration=True, parenthetical=True, concentration=True, description=True
        )

    def get_short_str(self):
        """Gets a short string describing the effect (for display in init summary)"""
        return self.get_str(
            duration=True, parenthetical=False, concentration=False, description=False
        )

    def get_str(
        self, duration=True, parenthetical=True, concentration=True, description=True
    ):
    ...
    def _duration_cmp(self):
        ...
        has_ticked_this_round = self.combat.index is not None and (
            (self.combat.index == index and not self.ticks_on_end)
            or self.combat.index > index
        )
        return (
            remaining,
            int(has_ticked_this_round),
            index,
            1 if self.ticks_on_end else 0,
        )
    ...
    def _duration_str(self):
        ...
        if math.isinf(remaining):
            ...
            if (
                self.combatant is None
                or self.combat is None
                or index == self.combatant.index
            ):  # our turn
                if ticks_on_end:
...
def parse_stat_choice(args, _):
    for i, arg in enumerate(args):
        if (
            arg == "True"
        ):  # hack: sadv/sdis on their own should be equivalent to -sadv/sdis all
            args[i] = arg = "all"
        else:
            args[i] = arg = arg[
                :3
            ].lower()  # only check first three arg characters against STAT_ABBREVIATIONS
