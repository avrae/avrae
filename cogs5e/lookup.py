...
class Lookup(commands.Cog):
    ...
    # devtest, todo remove me
    @commands.command(name='elt')
    async def entity_lookup_text(self, ctx, tid: int, eid: int):
        e = compendium.lookup_entity(tid, eid)
        await ctx.send(repr(e))

    @commands.command(name='ccforn')
    async def find_counter_for_entity_by_name(self, ctx, *, name):
        options = list(gamedata.compendium.compendium._entity_lookup.values())
        e = await search_and_select(ctx, options, name, lambda en: en.name,
                                    selectkey=lambda en: f"{en.name} ({en.entity_type})")
        await self.find_counter_for_entity(ctx, e.type_id, e.entity_id)

    @commands.command(name='ccfor')
    async def find_counter_for_entity(self, ctx, tid: int, eid: int):
        ...
        # this is just here in prototyping so I can break out with a return
        def discover_cc(e):
            eid = e.entity_id
            tid = e.type_id

            # cc with same id (if the tid is the cc tid)
            if tid == gamedata.LimitedUse.type_id and (lucc := consumables_by_lu_id.get(eid)) is not None:
                return lucc, '1 limiteduse id'
            ...
            # cc with same source entity
            if (source_cc := consumables_by_feature_id.get((tid, eid))) is not None:
                return source_cc, '2 source feature id'

            # cc with same name as source entity
            if (source_name_cc := consumables_by_name.get(e.name)) is not None:
                return source_name_cc, '3 source feature name'

            # cc with same name as cc granted by source entity
            for granted_lu in e.limited_use:
                if (lu_name_cc := consumables_by_name.get(granted_lu.name)) is not None:
                    return lu_name_cc, '4 source feature granted lu name'

            # cc with same root feature as source entity
            ...
            print(f"root feature: {root_feature!r}")
            import collections
            consumables_by_root = collections.defaultdict(lambda: [])
            for cc in char.consumables:
                if (cc_feature_root := compendium.lookup_entity(cc.ddb_source_feature_type, cc.ddb_source_feature_id)) is None:
                    continue
                depth = 0
            ...
            if consumables_by_root[root_feature]:
                return sorted(consumables_by_root[root_feature], key=lambda pair: pair[1])[0][0], '5 root feature'

            # cc with same name as any source entity parent, or same name as any cc granted by source entity parent
            for parent_feature in parents:
                if (parent_name_cc := consumables_by_name.get(parent_feature.name)) is not None:
                    return parent_name_cc, '6 parent feature name'

                for granted_lu in parent_feature.limited_use:
                    if (parent_lu_name_cc := consumables_by_name.get(granted_lu.name)) is not None:
                        return parent_lu_name_cc, '6.5 parent feature granted lu name'

            # error
            return "cc not found"

        await ctx.send(repr(discover_cc(e)))
