...
class CastSpell(Effect):
    ...
    async def preflight(self, autoctx):
        ...
        type_e10s = await autoctx.ctx.bot.ddb.get_accessible_entities(
            ctx=autoctx.ctx,
            user_id=autoctx.ctx.author.id,
            entity_type=gamedata.Spell.entity_type,
        )
    ...
    def run(self, autoctx):
        ...
        if spell.automation and spell.automation.effects:
            ...
            if self.casting_mod is not None:
                autoctx.evaluator.builtins["spell"] = autoctx.parse_intexpression(
                    self.casting_mod
                )
            ...
            if cast_level != spell.level and spell.higherlevels:
                autoctx.effect_queue(
                    f"**At Higher Levels**: {smart_trim(spell.higherlevels)}"
                )
        else:
    ...
    def children(self):
        if (
            ...
            and self.spell.automation
            and self.spell.automation.effects
        ):
