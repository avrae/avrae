...
class Condition(Effect):
    ...
    def run(self, autoctx):
        ...
        do_false = False
        try:
            condition_result = autoctx.parse_annostr(
                self.condition, is_full_expression=True
            )
            ...
            if self.error_behaviour == "true":
                ...
                raise StopExecution(
                    f"Error when evaluating condition `{self.condition}`:\n{e}"
                )
        else:
            if condition_result:
                do_true = True
        ...
        return ConditionResult(
            did_true=do_true, did_false=do_false, did_error=did_error, children=children
        )
