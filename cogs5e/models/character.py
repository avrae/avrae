...
class Character(StatBlock):
    ...
    async def from_bot_and_ids(cls, bot, owner_id: str, character_id: str):
        ...
        character = await bot.mdb.characters.find_one(
            {"owner": owner_id, "upstream": character_id}
        )
    ...
    def from_bot_and_ids_sync(cls, bot, owner_id: str, character_id: str):
        ...
        character = bot.mdb.characters.delegate.find_one(
            {"owner": owner_id, "upstream": character_id}
        )
        if character is None:
            raise NoCharacter()
    ...
    async def delete(ctx, owner_id, upstream):
        await ctx.bot.mdb.characters.delete_one(
            {"owner": owner_id, "upstream": upstream}
        )
        try:
    ...
    async def commit(self, ctx, do_live_integrations=True):
        ...
        if (
            self._live_integration is not None
            and do_live_integrations
            and self.options.sync_outbound
        ):
            self._live_integration.commit_soon(
                ctx
            )  # creates a task to commit eventually
    ...
    async def set_active(self, ctx):
        ...
        await ctx.bot.mdb.characters.update_many(
            {"owner": owner_id, "active": True}, {"$set": {"active": False}}
        )
    ...
    def hp(self, value):
        ...
        self._hp = max(
            0, value
        )  # reimplements the setter, but super().(property) = x doesn't work (py-14965)
        self.on_hp()
    ...
    def remove_known_spell(self, sb_spell):
        ...
        spell_in_book = next(
            s for s in self.spellbook.spells if s.name == sb_spell.name
        )
    ...
    async def select_consumable(self, ctx, name):
        return await search_and_select(
            ctx, self.consumables, name, lambda ctr: ctr.name
        )
    ...
    def update(self, old_character):
        ...
        new_cc_upstreams = set(
            con.live_id for con in self.consumables if con.live_id is not None
        )
        ...
        if sb.num_pact_slots is not None:
            ...
            sb.num_pact_slots = max(
                sb.num_pact_slots,
                sb.max_pact_slots
                - sb.get_max_slots(sb.pact_slot_level)
                + sb.get_slots(sb.pact_slot_level),
            )
