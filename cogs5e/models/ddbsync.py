...
class DDBSheetSync(LiveIntegration):
    ...
    async def _preflight(self):
        ...
        if self._ctx is None:
            raise ValueError(
                "Attempted to call DDB sheet sync method with no valid context"
            )
    ...
    async def _do_sync_hp(self):
        ...
        await self._ctx.bot.ddb.character.set_damage_taken(
            ddb_user=ddb_user,
            removed_hit_points=clamp(
                0, self.character.max_hp - self.character.hp, self.character.max_hp
            ),
        ...
        if any(sb.max_slots.values()):
            ...
            for l in range(1, 10):
                ...
                if l == sb.pact_slot_level:
                    slots_of_level_used -= (
                        sb.max_pact_slots - sb.num_pact_slots
                    )  # remove # of used pact slots
    ...
    async def _do_sync_consumable(self, consumable):
        ...
        await self._ctx.bot.ddb.character.set_limited_use(
            ...
            uses=clamp(
                0, consumable.get_max() - consumable.value, consumable.get_max()
            ),
            character_id=int(self.character.upstream_id),
        )
