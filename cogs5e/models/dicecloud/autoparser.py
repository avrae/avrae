    ...
    def get_automation(self, prop):
        ...
        if (damages := self.target_effects.damage) or self.target_effects.saves or self.meta.get('bonus') is not None:
            ...
            if (bonus := self.meta.get('bonus')):
                stack[-1].append({'type': 'attack', 'hit': stack.append([]) or stack[-1], 'miss': [], 'attackBonus': str(bonus)})
            for damage in damages:
                stack[-1].append({'type': 'damage', 'damage': f"{damage['damage']}[{damage['type']}]", 'overheal': False})
                
            # do save stuff
            for save in self.target_effects.saves:
                if (stat := save['stat'][:3].lower()) in STAT_ABBREVIATIONS:
                    stack[-1].append({'type': 'save', 'stat': stat, 'fail': stack.append([]) or stack[-1], 'success': []})
            for damage in self.target_effects.save_damage:
                stack[-1].append({'type': 'damage', 'damage': f"{damage['damage']}[{damage['type']}]", 'overheal': False})
                
        if (damages := self.self_effects.damage) or self.self_effects.saves or (self.meta.get('self') and self.meta.get('bonus') is not None):
            stack = []
            ...
            for save in self.self_effects.saves:
                if (stat := save['stat'][:3].lower()) in STAT_ABBREVIATIONS:
                    stack[-1].append({'type': 'save', 'stat': stat, 'fail': stack.append([]) and stack[-1], 'success': []})
            for damage in self.self_effects.save_damage:
                stack[-1].append({'type': 'damage', 'damage': f"{damage['damage']}[{damage['type']}]", 'overheal': False})
            
        log.debug(f"Damage for {prop['name']}: {self.target_effects.damage}, {self.target_effects.save_damage}, {self.self_effects.damage}, {self.self_effects.save_damage}")
    ...
    def parse(self, prop, *, initial = True, save = False):
        match prop['type']:
            ...
            case 'damage':
                ...
                effects = self.self_effects if self.meta.get('self') or prop.get('target') == 'self' else self.target_effects
                damage = {
                    'id': prop['_id'],
