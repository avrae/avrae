...
class DicecloudClient:
    ...
    async def add_spells(self, character, spells, spell_list=None):
        ...
        return await self.http.post(
            f"/api/character/{character.upstream[10:]}/spellList/{list_id}", [s.to_dicecloud() for s in spells]
        )

    async def create_character(
        self, name: str = "New Character", gender: str = None, race: str = None, backstory: str = None
    ):
