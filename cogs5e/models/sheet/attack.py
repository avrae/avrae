class Attack:
    ...
    def from_dict(cls, d):
        ...
        return cls(
            name=d.pop("name"),
            automation=automation.Automation.from_data(d.pop("automation")),
            **d,
        )
...
class AttackList:
    ...
    def build_str(self, caster):
        return "\n".join(
            atk.build_str(caster)
            for atk in sorted(self.attacks, key=lambda atk: atk.name)
        )
...
def old_to_automation(bonus=None, damage=None, details=None):
    ...
    if bonus is not None:
        ...
        attack_eff = [
            automation.Attack(hit=hit, miss=[], attackBonus=str(bonus).strip("{}<>"))
        ]
    else:
