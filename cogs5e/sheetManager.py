...
class SheetManager(commands.Cog):
    ...
    async def character(self, ctx, *, name: str = None):
        f"""If no character name is passed in, it will display the current character and information about the current channel, server and global characters if appropriate. Otherwise it switches the active character for a channel, server or global, whatever is most specific. 
