...
class SheetManager(commands.Cog):
    ...
    async def action(self, ctx, atk_name=None, *, args: str = ""):
        ...
        if isinstance(attack_or_action, Attack):
            result = await actionutils.run_attack(
                ctx, embed, args, caster, attack_or_action, targets, combat
            )
        else:
    ...
    async def attack_add(self, ctx, name, *args):
        ...
        conflict = next(
            (
                a
                for a in character.overrides.attacks
                if a.name.lower() == attack.name.lower()
            ),
            None,
        )
        if conflict:
    ...
    async def attack_delete(self, ctx, name):
        ...
        attack = await search_and_select(
            ctx, character.overrides.attacks, name, lambda a: a.name
        )
        if not (
            await confirm(
                ctx,
                f"Are you sure you want to delete {attack.name}? (Reply with yes/no)",
            )
        ):
    ...
    async def transferchar(self, ctx, user: discord.Member):
        ...
        conflict = await self.bot.mdb.characters.find_one(
            {"owner": str(user.id), "upstream": character.upstream}
        )
        if conflict:
    ...
    async def csettings(self, ctx, *args):
        ...
        if not args:
            settings_ui = ui.CharacterSettingsUI.new(
                ctx.bot, owner=ctx.author, character=char
            )
        ...
        for i, arg in enumerate(args):
            ...
            if arg in CHARACTER_SETTINGS:
                skip = True
                out.append(
                    CHARACTER_SETTINGS[arg].run(ctx, char, list_get(i + 1, None, args))
                )
    ...
    async def _confirm_overwrite(self, ctx, _id):
        ...
        conflict = await self.bot.mdb.characters.find_one(
            {"owner": str(ctx.author.id), "upstream": _id}
        )
        if conflict:
    ...
    async def _active_character_embed(ctx):
        ...
        embed.description = desc
        embed.set_footer(
            text=f"To change active characters, use {ctx.prefix}character <name>."
        )
...
async def send_ddb_ctas(ctx, character):
    ...
    gamelog_flag = await ctx.bot.ldclient.variation(
        "cog.gamelog.cta.enabled", ld_dict, False
    )
    ...
    if not embed.fields:
        return
    await ctx.send(embed=embed)
    await ctx.bot.rdb.setex(
        f"cog.sheetmanager.cta.seen.{ctx.author.id}", str(time.time()), 60 * 60 * 24 * 7
    )
