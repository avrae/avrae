...
class BeyondSheetParser(SheetLoaderABC):
    ...
    async def _get_character(self):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                f"{ENDPOINT}?charId={char_id}", headers=headers
            ) as resp:
                ...
                if resp.status == 200:
                    ...
                    if ddb_user is None:
                        raise ExternalImportError(
                            "This character is private. Link your D&D Beyond and Discord accounts"
                            " to import it!"
                        )
                    else:
                        raise ExternalImportError(
                    ...
                    raise ExternalImportError(
                        "This character does not exist. Are you using the right link?"
                    )
                elif resp.status == 429:
                    raise ExternalImportError(
                        "Too many people are trying to import characters! Please try again in "
                        "a few minutes."
                    )
                else:
                    raise ExternalImportError(
                        f"Beyond returned an error: {resp.status} - {resp.reason}"
                    )
        character["_id"] = char_id
        self.character_data = character
        self._is_live = (ddb_user is not None) and (
            ddb_user.user_id == str(character["ownerId"])
        )
        log.debug(character)
        return character
    ...
    def _get_skills(self) -> Skills:
        ...
        for skill_id, skill in self.character_data["skills"].items():
            ...
            out[SKILL_MAP[skill_id]] = Skill(
                skill["modifier"], prof_type, skill["bonus"], adv_type
            )
        ...
        for stat_key, skill in zip(constants.STAT_ABBREVIATIONS, constants.STAT_NAMES):
            ...
            prof_type = {1: 0, 2: 0.5, 3: 1, 4: 2}.get(
                stat_obj["modifierProficiency"], 0
            )
    ...
    def _get_saves(self) -> Saves:
        out = {}
        for stat_key, save_key in zip(
            constants.STAT_ABBREVIATIONS, constants.SAVE_NAMES
        ):
    ...
    def _get_custom_counters(self):
        ...
        for cons in self.character_data["consumables"]:
            ...
            source_feature = compendium.lookup_entity(
            ...
            if (
                cons["max"] and name
            ):  # don't make counters with a range of 0 - 0, or blank named counters
                out.append(
                    CustomCounter(
                        None,
                        name,
    ...
    def _get_attacks_and_actions(self):
        ...
        filtered_attacks = [
            attack
            for ((id, type_id), attack) in attacks
    ...
    def _process_attacks(self):
        ...
        def append(processed_attack, attack_data):
            ...
            attacks.append(
                ((str(attack_data["id"]), str(attack_data["typeId"])), processed_attack)
            )
    ...
    def _process_actions(self):
        ...
        for d_action in character_actions:
            ...
            try:
                parent_type_id, parent_id = int(d_action["componentTypeId"]), int(
                    d_action["componentId"]
                )
            ...
            if not (g_actions or parent_g_actions):
                activation_type = (
                    enums.ActivationType(d_action["activationType"])
    ...
    def _transform_attack(attack) -> Attack:
        ...
        if attack["saveDc"] is not None and attack["saveStat"] is not None:
            ...
            if attack["toHit"] is not None:
                the_attack = automation.Attack(
                    hit=[the_attack], attackBonus=str(attack["toHit"]), miss=[]
                )
            ...
            return Attack.new(
                attack["name"], attack["toHit"], attack["damage"] or "0", desc
            )
