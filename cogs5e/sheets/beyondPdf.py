...
class BeyondPDFSheetParser(SheetParser):
    ...
    async def get_character(self):
        ...
        def parsePDF():
            ...
            try:
                ...
                raise Exception('This is not a Beyond sheet!')
            for i in fields:
                field = resolve1(i)
    ...
    def get_sheet(self):
        ...
        sheet = {'type': 'beyond-pdf',
                 'version': 1, # fork of pdf v5
                 'stats': stats,
    ...
    def get_stats(self):
        ...
        for stat in ('strength', 'dexterity', 'constitution', 'wisdom', 'intelligence'):
            try:
                stats[stat + 'Mod'] = int(character.get(stat[:3].upper()))
                stats[stat] = int(character.get(stat[:3].upper() + 'mod'))
            except TypeError:
                raise MissingAttribute(stat)
        try:
            stats['charismaMod'] = int(character.get("CHA"))
            stats['charisma'] = int(character.get('CHamod'))
        except TypeError:
            raise MissingAttribute('charisma')
    ...
    def get_attack(self, atkIn):
        ...
        attack['name'] = character.get("Wpn Name" + (f" {atkIn}" if atkIn is not 1 else ''))
        attack['attackBonus'] = character.get(f"Wpn{atkIn} AtkBonus")
        attack['damage'] = character.get(f"Wpn{atkIn} Damage")
    ...
    def parse_manual_attack(self, atkIn):
        """Calculates and returns a dict."""
        if self.character is None: raise Exception('You must call get_character() first.')
        character = self.character
        attack = {'attackBonus': '0', 'damage': '0', 'name': ''}

        attack['name'] = atkIn.group(1)
        attack['attackBonus'] = atkIn.group(2)
        attack['damage'] = atkIn.group(3)
    ...
    def get_attacks(self):
        ...
        additional = self.character.get("AttacksSpellcasting") or ""
        add_atks = re.finditer(r"(.*) \| (.*) \| (.*)", additional)
        for a in add_atks:
            attacks.append(self.parse_manual_attack(a))
        return attacks
    ...
    def get_skills(self):
        ...
        if self.character is None: raise Exception('You must call get_character() first.')
        character = self.character
        skillslist = ['Acrobatics', 'Animal', 'Arcana', 'Athletics',
                      'ST Charisma', 'ST Constitution', 'Deception', 'ST Dexterity',
                      'History', 'Initiative', 'Insight', 'ST Intelligence',
                      'Intimidation', 'Investigation', 'Medicine', 'Nature',
                      'Perception', 'Performance', 'Persuasion', 'Religion',
                      'SleightofHand', 'Stealth', 'ST Strength', 'Survival', 'ST Wisdom']
        ...
        for stat in ('strength', 'dexterity', 'constitution', 'wisdom', 'intelligence', 'charisma'):
            skills[stat] = int(character.get(stat[:3].upper()))
    ...
    def get_spellbook(self):
        ...
        for lvl in range(1, 10):
            try:
                numSlots = int(self.character.get(f"SlotsTotal {lvl+18}") or 0)
            except ValueError:
                numSlots = 0
            spellbook['spellslots'][str(lvl)] = numSlots

        spellnames = set()
        for i in range(1014, 1100):
            s = self.character.get(f"Spells {i}")
            if s: spellnames.add(s)
        for i in range(10100, 10110):
            s = self.character.get(f"Spells {i}")
            if s: spellnames.add(s)
        for i in range(101010, 101014):
            s = self.character.get(f"Spells {i}")
            if s: spellnames.add(s)

        for spell in spellnames:
            s = next((s for s in c.spells if s.get('name') in spell), None)
            if s:
                spellbook['spells'].append(s.get('name'))

        try:
            spellbook['dc'] = int(self.character.get('SpellSaveDC  2', 0) or 0)
        except ValueError:
            pass

        try:
            spellbook['attackBonus'] = int(self.character.get('SpellAtkBonus 2', 0) or 0)
        except ValueError:
            pass
