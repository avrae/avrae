...
class DicecloudParser(SheetLoaderABC):
    ...
    def get_coinpurse(self):
        ...
        for i in self.character_data.get("items"):
            if re.fullmatch(
                r"^((plat(inum)?|gold|electrum|silver|copper)( pieces?)?|(pp|gp|ep|sp|cp))", i["name"], re.IGNORECASE
            ):
    ...
    def get_skills_and_saves(self) -> (Skills, Saves):
        ...
        for effect in self.character_data.get("effects", []):
            if effect.get("stat") in NAME_SET and effect.get("enabled", True) and not effect.get("removed", False):
        ...
        for save in SAVE_NAMES:
            ...
            saves[save] = Skill(int(self.calculate_stat(save, base=base_val)), prof=prof_mult, adv=adv)
        ...
        if atk_dict.get("parent", {}).get("collection") == "Spells":
            ...
            try:
                ...
                try:
                    ...
                    try:
                        temp_names["attackBonus"] = int(self.evaluator.eval(spellListObj.get("attackBonus")))
