...
class GoogleSheet(SheetLoaderABC):
    ...
    async def _init_gsheet_client():
        with GoogleSheet._client_lock():
            ...
            try:
                GoogleSheet.g_client = await asyncio.get_event_loop().run_in_executor(
                    None, _
                )
            except:
                GoogleSheet._client_initializing = False
    ...
    def _gchar(self):
        ...
        if "1.3" in vcell:
            ...
            self.version = (
                (2, 1) if "2.1" in vcell else (2, 0) if "2" in vcell else (1, 0)
            )
            if self.version >= (2, 1):
    ...
    async def load_character(self, ctx, args):
        ...
        return await asyncio.get_event_loop().run_in_executor(
            None, self._load_character, owner_id, args
        )
    ...
    def get_coinpurse(self):
        ...
        for c_type in COIN_TYPES:
            if self.version >= (2, 1):
                coins[c_type] = int(
                    self.inventory.value(COIN_TYPES[c_type]["gSheet"]["v2"]) or 0
                )
            else:
                coins[c_type] = int(
                    self.character_data.value(COIN_TYPES[c_type]["gSheet"]["v14"]) or 0
                )
    ...
    def get_attacks(self):
        ...
        if self.additional:
            for rownum in range(3, 14):  # sht2, B3:B13; W3:W13
                additional = self.parse_attack(
                    f"B{rownum}", f"I{rownum}", f"M{rownum}", self.additional
                )
                other = self.parse_attack(
                    f"W{rownum}", f"AD{rownum}", f"AH{rownum}", self.additional
                )
    ...
    def get_spellbook(self):
        ...
        for spell_name, prepared in potential_spells:
            ...
            if len(spell_name) > 2 and spell_name not in IGNORED_SPELL_VALUES:
                ...
                result, strict = search(
                    compendium.spells, spell_name, lambda sp: sp.name, strict=True
                )
        ...
        if (
            spell_mod_value
        ):  # it might be in the form of a ability name, or an int, wjdk
            try:
        ...
        spellbook = Spellbook(
            slots, slots, spells, dc, sab, self.total_level, spell_mod
        )
        return spellbook
    ...
    def _get_potential_spells(self):
        ...
        if "noprep" in self.args:
            # spells C96:AH143
            potential_spells = [
                (sn, True) for sn in self.character_data.value_range("D96:AH143")
            ]
            if self.additional:
                potential_spells.extend(
                    (sn, True) for sn in self.additional.value_range("D17:AH64")
                )
        else:
            potential_spells = []
            for spell_col, prep_col, rows in SPELL_RANGES:
                potential_spells.extend(
                    self._process_spell_range(
                        spell_col, prep_col, rows, self.character_data
                    )
                )
            ...
            if self.additional:
                for spell_col, prep_col, rows in SPELL_RANGES_ADDITIONAL:
                    potential_spells.extend(
                        self._process_spell_range(
                            spell_col, prep_col, rows, self.additional
                        )
                    )
