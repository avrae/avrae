...
def run_check(skill_key, caster, args, embed):
    ...
    if any(args.last(s, type_=bool) for s in STAT_ABBREVIATIONS):
        ...
        mod = (
            mod
            - caster.stats.get_mod(SKILL_MAP[skill_key])
            + caster.stats.get_mod(base)
        )
    ...
    return CheckResult(
        rolls=result.rolls, skill=skill, skill_name=skill_name, skill_roll_result=result
    )
...
def run_save(save_key, caster, args, embed):
    ...
    if isinstance(caster, init.Combatant):
        ...
        if "all" in sadv_effects or stat in sadv_effects:
            args[
                "adv"
            ] = True  # Because adv() only checks last() just forcibly add them
        if "all" in sdis_effects or stat in sdis_effects:
            args["dis"] = True

    result = _run_common(save, args, embed, rr_format="Save {}")
    return SaveResult(
        rolls=result.rolls, skill=save, skill_name=stat_name, skill_roll_result=result
    )
...
def _run_common(skill, args, embed, mod_override=None, rr_format="Check {}"):
    ...
    for i in range(iterations):
        ...
        # set up dice
        roll_str = skill.d20(
            base_adv=adv, reroll=ro, min_val=mc, mod_override=mod_override
        )
        if b is not None:
    ...
    if iterations > 1 and dc:
        embed.set_footer(
            text=f"{num_successes} Successes | {iterations - num_successes} Failures"
        )
    elif dc:
    ...
    return SkillRollResult(
        rolls=results, iterations=iterations, dc=dc, successes=num_successes
    )
