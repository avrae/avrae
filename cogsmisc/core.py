...
class Core(commands.Cog):
    ...
    async def ping(self, ctx):
        ...
        wsping = (
            floor(self.bot.latency * 1000) if isfinite(self.bot.latency) else "Unknown"
        )
        await pong.edit(
    ...
    async def about(self, ctx):
        ...
        statKeys = (
            "dice_rolled_life",
            "spells_looked_up_life",
            "monsters_looked_up_life",
            "commands_used_life",
            "items_looked_up_life",
            "rounds_init_tracked_life",
            "turns_init_tracked_life",
        )
        for k in statKeys:
        ...
        embed.add_field(
            name="Uptime",
            value=str(timedelta(seconds=round(time.monotonic() - self.start_time))),
        )
        motd = random.choice(
            [
    ...
    async def ddb(self, ctx):
        ...
        if ddb_user is None:
            ...
            embed.set_footer(
                text="Already linked your account? It may take up to 15 minutes for Avrae to recognize "
                "the link."
            )
        ...
        desc = await self.bot.ldclient.variation(
            "command.ddb.desc", ddb_user.to_ld_dict(), default_desc
        )
