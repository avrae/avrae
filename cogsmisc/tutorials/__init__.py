...
class Tutorials(commands.Cog):
    ...
    async def tutorial(self, ctx, *, name=None):
        ...
        if user_state is not None:
            ...
            key, tutorial = await search_and_select(
                ctx, list(self.tutorials.items()), name, lambda tup: tup[1].name
            )
    ...
    async def tutorial_list(self, ctx):
        ...
        for tutorial in self.tutorials.values():
            embed.add_field(
                name=tutorial.name, value=tutorial.description, inline=False
            )
    ...
    async def tutorial_skip(self, ctx):
        ...
        result = await confirm(
            ctx,
            "Are you sure you want to skip the current tutorial objective? (Reply with yes/no)",
        )
        if not result:
    ...
    async def tutorial_end(self, ctx):
        ...
        result = await confirm(
            ctx,
            "Are you sure you want to end the current tutorial? (Reply with yes/no)",
        )
        if not result:
            return await ctx.send("Ok, aborting.")
