...
from cogs5e.initiative import CombatNotFound, MonsterCombatant
    ...
    *20 minutes*
    ...
    class HitPoints2(TutorialState):
        ...
        async def listener(self, ctx, state_map):
            if ctx.channel.id != state_map.persist_data.get("channel_id"):
                return
            if ctx.command in (ctx.bot.get_command("i hp set"), ctx.bot.get_command("i hp")):
                the_combat = await ctx.get_combat()
                woofer = get_the_woofer(the_combat)
                if woofer is None:
                    await ctx.send(
    ...
    class HitPoints3(TutorialState):
        ...
        async def listener(self, ctx, state_map):
            if ctx.channel.id != state_map.persist_data.get("channel_id"):
                return
            if ctx.command in (ctx.bot.get_command("i hp max"), ctx.bot.get_command("i hp")):
                the_combat = await ctx.get_combat()
                woofer = get_the_woofer(the_combat)
                if woofer is None:
                    await ctx.send(
