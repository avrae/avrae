...
class CharacterServiceClient(BaseClient):
    ...
    async def set_limited_use(self, ddb_user, id: int, entity_type_id: int, uses: int, character_id: int):
        data = {"id": id, "entityTypeId": entity_type_id, "uses": uses, "characterId": character_id}
    ...
    async def set_death_saves(self, ddb_user, success_count: int, fail_count: int, character_id: int):
        data = {"successCount": success_count, "failCount": fail_count, "characterId": character_id}
        return await self.put(ddb_user, "/life/death-saves", json=data)

    # ==== Spell ====
    async def set_pact_magic(
        self, ddb_user, level1: int, level2: int, level3: int, level4: int, level5: int, character_id: int
    ):
        data = {
            "level1": level1,
            "level2": level2,
    ...
    async def set_currency(self, ddb_user, pp: int, gp: int, ep: int, sp: int, cp: int, character_id: int):
        data = {"cp": cp, "sp": sp, "ep": ep, "gp": gp, "pp": pp, "characterId": character_id}
