...
class RollContext:
    ...
    def from_dict(cls, d):
        return cls(
            d.get("entityId"), d.get("entityType"), d.get("name"), d.get("avatarUrl")
        )
...
class RollResult:
    ...
    def from_d20_and_dice_notation(cls, result: d20.RollResult, dice_notation):
        values = [
            die.die_value for dt in dice_notation.set for die in dt.dice
        ]  # multi-loop drifting!!!
...
class DiceNotation:
    ...
    def d20_ast(self, **kwargs):
        ...
        return self._build_tree(
            module_base=d20.ast, child_method=lambda dt: dt.d20_ast, **kwargs
        )
    ...
    def d20_expr(self, **kwargs):
        ...
        """
        return self._build_tree(
            module_base=d20, child_method=lambda dt: dt.d20_expr, **kwargs
        )
    ...
    def _build_tree(self, module_base, child_method, comment=None):
        ...
        if not self.set:
            return module_base.Expression(
                module_base.Literal(self.constant), comment=comment
            )
