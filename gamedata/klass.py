...
class Class(Sourced):
    ...
    def from_data(cls, d):
        ...
        inst.levels = [
            [ClassFeature.from_data(cf, inst) for cf in lvl] for lvl in d["levels"]
        ]
        inst.optional_features = [
            ClassFeature.from_data(ocf, inst) for ocf in d["optional_features"]
        ]
        return inst
...
class Subclass(Sourced):
    ...
    def __init__(
        self, name, levels, optional_features, parent=None, description=None, **kwargs
    ):
        """
        :type name: str
        :type levels: list[list[ClassFeature]]
    ...
    def from_data(cls, d, parent_class):
        ...
        inst.levels = [
            [ClassFeature.from_data(cf, source_class=inst) for cf in lvl]
            for lvl in d["levels"]
        ]
        inst.optional_features = [
            ClassFeature.from_data(ocf, source_class=inst)
...
class ClassFeature(LimitedUseGrantorMixin, DescribableMixin, Sourced):
    ...
    def from_data(cls, d, source_class, **kwargs):
        ...
        entitlement_entity_type = d.get(
            "entitlementEntityType", kwargs.pop("entitlement_entity_type", "class")
        )
        ...
        if "options" in d:
            inst.options = [
                ClassFeatureOption.from_data(o, source_class, inst)
                for o in d["options"]
            ]
...
class ClassFeatureOption(ClassFeature):
    ...
    def from_data(cls, d, source_class, class_feature=None, **kwargs):
        return super().from_data(
            ...
            entitlement_entity_id=d.get(
                "entitlementEntityId", class_feature.entitlement_entity_id
            ),
            entitlement_entity_type=d.get(
                "entitlementEntityType", class_feature.entitlement_entity_type
            ),
            **kwargs
        )
