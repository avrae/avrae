...
async def available(ctx, entities, entity_type, user_id=None):
    ...
    return [
        e for e in entities if e.is_free or e.entitlement_entity_id in available_ids
    ]
...
def can_access(entity, available_ids=None):
    return (
        entity.is_free
        or available_ids is not None
        and entity.entitlement_entity_id in available_ids
        or entity.homebrew
    )
...
async def handle_required_license(ctx, err):
    ...
    await ctx.bot.mdb.analytics_nsrd_lookup.update_one(
        {"type": result.entity_type, "name": result.name},
        {"$inc": {"num_lookups": 1}},
        upsert=True,
    )
    ...
    if not err.has_connected_ddb:
        ...
        embed.set_footer(
            text="Already unlocked? It may take up to a minute for Avrae to recognize the purchase."
        )
...
def handle_source_footer(
    embed, sourced, text=None, add_source_str=True, allow_overwrite=False
):
    """
...
async def select_spell_full(ctx, name, *args, extra_choices=None, **kwargs):
    ...
    return await search_and_select(
        ctx, choices, name, lambda e: e.name, *args, **kwargs
    )
