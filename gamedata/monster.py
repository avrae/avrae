...
class Monster(StatBlock, Sourced):
    ...
    def from_bestiary(cls, data, source):
        ...
        if "display_resists" in data:
            data["display_resists"] = Resistances.from_dict(
                data["display_resists"], smart=False
            )
        else:
    ...
    def to_dict(self):
        return {
            ...
            "legactions": [t.to_dict() for t in self.legactions],
            ...
            "spellbook": self.spellbook.to_dict(),
    ...
    def get_hidden_stat_array(self):
        ...
        for i, stat in enumerate(
            ...
            if stat <= 3:
                ...
                stats[i] = "Ludicrous"
        return (
            f"**STR**: {stats[0]} **DEX**: {stats[1]} **CON**: {stats[2]}\n"
    ...
    for dmgtype in resists:
        if isinstance(dmgtype, str):
            ...
            if "special" in dmgtype:
                ...
                rs = parse_resists(
                    dmgtype.get("resist")
                    or dmgtype.get("immune")
                    or dmgtype.get("vulnerable")
                )
                if notated:
...
class MonsterSpellbook(Spellbook):
    def __init__(self, *args, at_will=None, daily=None, daily_max=None, **kwargs):
        ...
        self.daily = daily
        self.daily_max = (
            daily_max or daily.copy()
        )  # only monsters can have daily, and we'll init their max to immutable daily

    def to_dict(self):
        d = super().to_dict()
        d.update(
            {"at_will": self.at_will, "daily": self.daily, "daily_max": self.daily_max}
        )
        return d
    ...
    def slots_str(self, level: int = None):
        ...
        for spell, remaining in self.daily.items():
            before.append(
                f"**{spell}**: {bubble_format(remaining, self.daily_max[spell])}"
            )
        before = "\n".join(before)
        return f"{before}\n{slots}".strip()
    ...
    def cast(self, spell, level, pact=True):
        ...
        if daily_key is not None and daily_value > 0:
            ...
            raise CounterOutOfBounds(
                f"You do not have any remaining casts of {spell.name}."
            )
