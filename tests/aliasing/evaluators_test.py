...
async def test_yaml_types(draconic_evaluator):
    ...
    result = draconic_evaluator.eval(
        "load_yaml('key: /X17unp5WZmZgAAAOfn515eXvPz7Y6O')"
    )
...
async def test_anchors(draconic_evaluator):
    ...
    assert type(result) is draconic_evaluator._dict
    assert (
        type(result["template"]["tags"])
        is type(result["response"]["tags"])
        is draconic_evaluator._list
    )
