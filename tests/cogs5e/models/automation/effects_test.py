...
class TestIEffect:
    ...
    async def test_ieffect_teardown(
        self, avrae, dhttp
...
async def test_usecounter_e2e(character, avrae, dhttp):
    ...
    await dhttp.drain()
    char = await active_character(avrae)
    assert char.spellbook.get_slots(3) == 0 or char.spellbook.get_slots(
        3
    ) < char.spellbook.get_max_slots(3)
...
async def test_usecounter_serialize():
    ...
    result = automation.UseCounter(
        automation.effects.usecounter.SpellSlotReference(1), "1"
    ).to_dict()
    assert json.dumps(result)

    result = automation.UseCounter(
        automation.effects.usecounter.AbilityReference(12168134, 75), "1"
    ).to_dict()
    assert json.dumps(result)
