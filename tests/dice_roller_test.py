from cogs5e.funcs.dice import old_roll, DiceResult


def test_roll():
    assert type(old_roll("1d20")) == DiceResult
    assert 0 < old_roll("1d20").total < 21
    assert old_roll("3+4*(9-2)").total == 31
...
def test_complex_rolls():
    r = old_roll("10d6rol5mi6ma1k1[annotation] some comments")
    assert r.total == 10
    assert r.crit == 0
    assert "some comments" in r.result

    r = old_roll("5d6kh4e0kl3")
    assert 3 <= r.total <= 18
...
def test_infinite_loops():
    ...
    r = old_roll("1d1e1rr1e1rr1")
    assert r.total == 0  # error


def test_randomness():
    rolls = [old_roll("1d1000").total for _ in range(100)]
