...
class CharacterSettingsUI(CharacterSettingsMenuBase):
    ...
    async def gameplay_settings(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
        await self.defer_to(_GameplaySettingsUI, interaction)

    @disnake.ui.button(
        label="Character Sync Settings", style=disnake.ButtonStyle.primary
    )
    async def character_sync_settings(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
    ...
    async def get_content(self):
        embed = embeds.EmbedWithCharacter(
            self.character, title=f"Character Settings for {self.character.name}"
        )
        embed.add_field(
...
class _CosmeticSettingsUI(CharacterSettingsMenuBase):
    ...
    async def select_color(
        ...
        try:
            ...
            await interaction.send(
                "No valid color found. Press `Select Color` to try again.",
                ephemeral=True,
            )
        else:
    ...
    @disnake.ui.button(
        label="Toggle Show Character Image", style=disnake.ButtonStyle.primary, row=1
    )
    async def toggle_show_character_image(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
    ...
    @disnake.ui.button(
        label="Toggle Compact Coin Display", style=disnake.ButtonStyle.primary, row=2
    )
    async def toggle_compact_coin_display(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
...
class _GameplaySettingsUI(CharacterSettingsMenuBase):
    @disnake.ui.select(
        placeholder="Select New Crit Range", options=_CRIT_RANGE_SELECT_OPTIONS
    )
    async def crit_range_select(
    ...
    @disnake.ui.select(
        placeholder="Select Extra Crit Dice", options=_CRIT_DICE_SELECT_OPTIONS, row=1
    )
    async def crit_dice_select(
        self, select: disnake.ui.Select, interaction: disnake.Interaction
    ):
    ...
    @disnake.ui.select(
        placeholder="Select Reroll", options=_REROLL_SELECT_OPTIONS, row=2
    )
    async def reroll_select(
        self, select: disnake.ui.Select, interaction: disnake.Interaction
    ):
    ...
    @disnake.ui.button(
        label="Toggle Ignore Crits", style=disnake.ButtonStyle.primary, row=3
    )
    async def toggle_ignore_crits(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
    ...
    @disnake.ui.button(
        label="Toggle Reliable Talent", style=disnake.ButtonStyle.primary, row=3
    )
    async def toggle_reliable_talent(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
    ...
    @disnake.ui.button(
        label="Toggle Short Rest Slots", style=disnake.ButtonStyle.primary, row=3
    )
    async def toggle_srslots(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
