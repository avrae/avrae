...
class ServerSettingsUI(ServerSettingsMenuBase):
    ...
    @disnake.ui.button(
        label="Inline Rolling Settings", style=disnake.ButtonStyle.primary
    )
    async def inline_rolling_settings(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
        await self.defer_to(_InlineRollingSettingsUI, interaction)

    @disnake.ui.button(
        label="Miscellaneous Settings", style=disnake.ButtonStyle.primary
    )
    async def miscellaneous_settings(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
...
class _LookupSettingsUI(ServerSettingsMenuBase):
    ...
    @disnake.ui.button(
        label="Toggle Monsters Require DM", style=disnake.ButtonStyle.primary, row=1
    )
    async def toggle_dm_required(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
    ...
    @disnake.ui.button(
        label="Toggle Direct Message DMs", style=disnake.ButtonStyle.primary, row=1
    )
    async def toggle_pm_dm(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
    ...
    @disnake.ui.button(
        label="Toggle Direct Message Results", style=disnake.ButtonStyle.primary, row=1
    )
    async def toggle_pm_result(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
    ...
    async def _text_select_dm_roles(
        ...
        try:
            ...
            if input_msg.content == "reset":
                await interaction.send(
                    "The DM roles have been updated.", ephemeral=True
                )
                return None
    ...
    def _refresh_dm_role_select(self):
        ...
        for role in reversed(self.guild.roles):  # display highest-first
            selected = (
                self.settings.dm_roles is not None and role.id in self.settings.dm_roles
            )
            self.select_dm_roles.add_option(
...
class _MiscellaneousSettingsUI(ServerSettingsMenuBase):
    # ==== ui ====
    @disnake.ui.button(
        label="Toggle DDB Campaign Message", style=disnake.ButtonStyle.primary, row=1
    )
    async def toggle_campaign_cta(
        self, _: disnake.ui.Button, interaction: disnake.Interaction
    ):
