...
class CSetting:  # character settings
    def __init__(
        self, setting_key, type_, description=None, default=None, display_func=None
    ):
        self.character = None
        self.ctx = None
    ...
    def reset(self):
        setattr(
            ...
            self.setting_key,
            CharacterSettings.__fields__[self.setting_key].default,
        )
    ...
    def set(self, new_value):
        ...
        return (
            f"\u2705 {self.description.capitalize()} set to {self.display_func(val)}.\n"
        )


CHARACTER_SETTINGS = {
    "color": CSetting(
        "color", "color", default="random", display_func=lambda val: f"#{val:06X}"
    ),
    "criton": CSetting(
        "crit_on",
        "number",
    ...
    "critdice": CSetting(
        "extra_crit_dice", "number", description="extra crit dice", default=0
    ),
    "talent": CSetting(
        "talent",
        "boolean",
